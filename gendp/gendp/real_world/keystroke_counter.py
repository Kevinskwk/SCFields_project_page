...
try:
    ...
    class Listener:
        def __init__(self, *args, **kwargs):
            self._pynput_import_error = _PYNPUT_IMPORT_ERROR

        def __enter__(self):
            raise ImportError(
                "pynput keyboard listener is unavailable. Real robot keyboard "
                "control requires a supported display/session."
            ) from self._pynput_import_error
