...
class TactileFieldSensor(TactileBase):
    ...
    def generate_tactile_points(self, elastomer_parent_urdf_path, elastomer_link_name, elastomer_tip_link_name,
                                elastomer_actor_name, num_divs=[20, 25], dx=0.003, visualize=False):
        """
        ...
        Args:
            ...
            dx (float): Distance between tactile points.
        ...
        # div_sz = (elastomer_dims[axis_idxs] - np.array(margins) * 2.) / (np.array(num_divs) + 1)
        # tactile_points_dx = min(div_sz)
        # tactile_points_dx = 0.02075 / 6.
        tactile_points_dx = dx
    ...
    def load_sdf_oracle_of_indenter(self, indenter_urdf_path, indenter_actor_name, indenter_rb_name):
        ...
        indenter_mesh_local_tf = (torch.tensor(tf_quat, dtype=torch.float, device=self.device).unsqueeze(0), torch.tensor(tf_pos, dtype=torch.float, device=self.device).unsqueeze(0))
    ...
    def initialize_penalty_based_tactile(self, num_divs, dx):
        ...
        self.tactile_pos_local, self.tactile_quat_local = self.generate_tactile_points(
            ...
            num_divs=num_divs, dx=dx, visualize=False)
