...
class Fusion():
    def __init__(self, num_cam, feat_backbone='dinov2', device='cuda:0', dtype=torch.float32):
        ...
        if not os.path.exists(grounded_checkpoint):
            raise FileNotFoundError(f"Missing GroundingDINO checkpoint: {grounded_checkpoint}")
        ...
        if not os.path.exists(XMem_path):
            raise FileNotFoundError(f"Missing XMem checkpoint: {XMem_path}")
        xmem_config = {
            'model': XMem_path,
...
def test_grounded_sam():
    img = cv2.imread("${SCFIELDS_ROOT}/left_bottom_view_color_0.png")
